// Update medicin details
import javax.swing.*;
import java .awt.*;
import java.awt.event.*;
import java.sql.*;

class UpdateMedicine implements ActionListener,KeyListener
{
JFrame f;
...
try
{
st=con.prepareStatement("Update medicine set m_name=?,c_name=?,MRP=?,unit_price=?,type=?,menufacturing_date=?,expire_date=? where m_id=?");
st.setString(1,mn);
st.setString(2,cn);
st.setString(3,mrp);
st.setString(4,up);
st.setString(5,typ);
st.setString(6,sdate);
st.setString(7,edate);
st.setString(8,id);
st.executeUpdate();
...
class UpdateMedicineDemo
{
public static void main(String[] s)
{
UpdateMedicine m=new UpdateMedicine();
}
}